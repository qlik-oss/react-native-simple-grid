...
function TableHeadWrapper({
  ...
  const setHeadRowHeight = useContextSelector(
    TableContext,
    (value) => value.setHeadRowHeight,
  );
  const isFocusInHead = useContextSelector(
    TableContext,
    (value) => value.focusedCellCoord[0] === 0,
  );
  const setFocusedCellCoord = useContextSelector(
    TableContext,
    (value) => value.setFocusedCellCoord,
  );
  const headerStyle = useMemo(
    () => getHeaderStyle(layout, theme),
    [layout, theme],
  );
  ...
  useEffect(() => {
    ...
    headRowRef.current?.getBoundingClientRect().height,
  ]);
        ...
        {columns.map((column, columnIndex) => {
          ...
          return (
            <TableCell
              ...
              tabIndex={tabIndex}
              aria-sort={
                isCurrentColumnActive ? `${column.sortDirection}ending` : null
              }
