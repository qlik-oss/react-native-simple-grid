...
function PaginationContent({
  ...
  const {totalRowCount, totalColumnCount, totalPages, paginationNeeded} =
    tableData;
  ...
  const width = footerContainer
    ? footerContainer.getBoundingClientRect().width
    : rect.width;
  ...
  const displayedRowsText = translator.get(
  ...
  const handleChangeRowsPerPage = (evt) => {
    ...
    announce({
      keys: [['SNTable.Pagination.RowsPerPageChange', evt.target.value]],
  ...
  const handleLastButtonTab = keyboard.enabled
    ? (event) => handleLastTab(event, isSelectionMode, keyboard)
    : null;
  ...
  const getButton = (disabledCondition, pageNumber, type, onKeyDown = null) => {
    ...
    return (
      <StyledIconButton
        ...
        title={
          !constraints.passive
            ? translator.get(`SNTable.Pagination.${type}`)
            : undefined
        }
        tabIndex={tabIndex}
        onKeyDown={onKeyDown}>
  ...
  const getDropdown = (name, value, options, handleChange) => {
    ...
    return (
      <FormControl sx={{px: 2.5}}>
        <InputLabel
          sx={{color: paginationTheme.color}}
          htmlFor={id}
          shrink={false}>
  ...
  return (
    <>
      ...
      {shouldShow('selectPage', width) &&
        getDropdown('SelectPage', page, pageOptions, handleSelectPage)}
      ...
      {getButton(
        ...
        page + 1,
        'NextPage',
        !showFirstAndLast ? handleLastButtonTab : null,
      )}
      {showFirstAndLast &&
        getButton(onLastPage, totalPages - 1, 'LastPage', handleLastButtonTab)}
    </>
  );
}
