...
export function getColor(defaultColor, theme, color = {}) {
  ...
  return !resolvedColor || resolvedColor === 'none'
    ? defaultColor
    : resolvedColor;
}
...
export const getAutoFontColor = (backgroundColor) =>
  isDarkColor(backgroundColor)
    ? STYLING_DEFAULTS.WHITE
    : STYLING_DEFAULTS.FONT_COLOR;

export const getBaseStyling = (styleObj, objetName, theme) => {
  const fontFamily = theme.getStyle(
    'object',
    `straightTable.${objetName}`,
    'fontFamily',
  );
  const color = theme.getStyle('object', `straightTable.${objetName}`, 'color');
  const fontSize = theme.getStyle(
    'object',
    `straightTable.${objetName}`,
    'fontSize',
  );
  ...
  const baseStyle = {
    fontFamily,
    color: isSet(styleObj?.fontColor)
      ? getColor(STYLING_DEFAULTS.FONT_COLOR, theme, styleObj.fontColor)
      : color,
  ...
  Object.keys(baseStyle).forEach(
    (key) => baseStyle[key] == null && delete baseStyle[key],
  );
  return baseStyle;
};

export function getHeaderStyle(layout, theme) {
  const themeComponent = layout?.components?.find(
    (comp) => comp.key === 'theme',
  );
  const header = themeComponent?.header
  ...
  headerStyle.sortLabelColor =
    headerStyle.color ??
    (isDarkColor(headerStyle?.backgroundColor)
...
export function getBodyCellStyle(layout, theme) {
  ...
  const content = themeComponent?.content
  ...
  const hoverFontColorFromTheme = theme.getStyle(
    'object',
    '',
    'straightTable.content.hover.color',
  );
  ...
  const isHoverFontColorSet =
    isSet(hoverFontColorFromLayout) || !!hoverFontColorFromTheme;
  const isHoverBackgroundColorSet =
    isSet(hoverBackgroundColorFromLayout) || !!hoverBackgroundColorFromTheme;
  const isHoverFontOrBackgroundColorSet =
    isHoverFontColorSet || isHoverBackgroundColorSet;
  ...
  if (isSet(hoverBackgroundColorFromLayout)) {
    ...
    hoverBackgroundColor = getColor(
      ...
      theme,
      hoverBackgroundColorFromLayout,
    );
  ...
  const hoverFontColor = isHoverFontOrBackgroundColorSet
    ? getColor(
        ...
        theme,
        isSet(hoverFontColorFromLayout)
          ? hoverFontColorFromLayout
          : hoverFontColorFromTheme,
      ) // case 1 or 3 or 4
    : ''; // case 2;
...
export function getColumnStyle(styling, qAttrExps, stylingInfo) {
  ...
  if (columnColors.cellBackgroundColor && !columnColors.cellForegroundColor) {
    columnColors.cellForegroundColor = getAutoFontColor(
      columnColors.cellBackgroundColor,
    );
  }
