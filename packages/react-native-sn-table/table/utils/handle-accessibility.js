export const getCellElement = (rootElement, cellCoord) =>
  rootElement
    .getElementsByClassName('sn-table-row')
    [cellCoord[0]]?.getElementsByClassName('sn-table-cell')[cellCoord[1]];

export const findCellWithTabStop = (rootElement) =>
  rootElement.querySelector("td[tabindex='0'], th[tabindex='0']");
...
export const handleClickToFocusBody = (
  ...
  const adjustedRowIdx =
    totalsPosition === 'top' ? rawRowIdx + 2 : rawRowIdx + 1;
  removeAndFocus(
    [adjustedRowIdx, rawColIdx],
    rootElement,
    setFocusedCellCoord,
    keyboard,
  );
...
export const handleMouseDownLabelToFocusHeadCell = (
  ...
  updateFocus({
    focusType: 'focus',
    cell: getCellElement(rootElement, [0, columnIndex]),
  });
};
