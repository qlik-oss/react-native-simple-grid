export const handleHorizontalScroll = (evt, isRTL, memoedContainer) => {
  ...
  const maxScrollableWidth =
    memoedContainer.scrollWidth - memoedContainer.offsetWidth;
  ...
  if (isRTL) {
    scrolledDistance = maxScrollableWidth + scrolledDistance;
  }
  if (
    maxScrollableWidth > 0 &&
    (scrolledDistance < 0 || scrolledDistance > maxScrollableWidth + 1)
  ) {
    evt.preventDefault();
    scrollLeft = isRTL
...
export const handleNavigateTop = ({
  ...
  if (focusedCellCoord[0] < MIN_ROW_COUNT) {
    ...
    const tableHead =
      rootElement.getElementsByClassName('sn-table-head-cell')[0];
    const rowElements = rootElement.getElementsByClassName('sn-table-row');
    const cell = rowElements[x]?.getElementsByClassName('sn-table-cell')[y];

    if (
      cell.offsetTop - tableHead.offsetHeight - cell.offsetHeight <=
