...
export interface SelectMultiValuesAction
  extends Action<SelectionActions.SELECT_MULTI_VALUES> {}
...
export const getCellSelectionState = (
  cell: TableCell,
  value: ContextValue,
): SelectionStates => {
  const {
...
export const getSelectedRows = (
  ...
  if (evt.shiftKey && key.includes('Arrow')) {
    ...
    selectedRows[
      key === 'ArrowDown' ? cell.nextQElemNumber : cell.prevQElemNumber
...
const selectCell = (
  ...
  const isSelectMultiValues =
    evt.shiftKey && (evt as React.KeyboardEvent)?.key.includes('Arrow');
