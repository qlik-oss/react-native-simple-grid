...
export const tableThemeColors = (theme, rootElement) => {
  ...
  const objectBackgroundColorFromCSS =
    qvInnerObject && window.getComputedStyle(qvInnerObject).backgroundColor;

  const qvPanelSheet = rootElement?.closest('.qv-panel-sheet');
  const sheetBackgroundColorFromCSS =
    qvPanelSheet && window.getComputedStyle(qvPanelSheet).backgroundColor;

  const tableBackgroundColorFromTheme = theme.getStyle(
    '',
    '',
    'object.straightTable.backgroundColor',
  );

  const backgroundColorFromTheme = theme.getStyle(
    'object',
    'straightTable',
    'backgroundColor',
  );
  ...
  const pagination = {
    ...
    iconColor: isBackgroundDarkColor
      ? 'rgba(255, 255, 255, 0.9)'
      : 'rgba(0, 0, 0, 0.54)',
    disabledIconColor: isBackgroundDarkColor
