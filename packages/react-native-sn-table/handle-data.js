...
import cssColors from './table/utils/css-colors';
...
export function getHighestPossibleRpp(width, rowsPerPageOptions) {
  const highestPossibleOption = [...rowsPerPageOptions]
    .reverse()
    .find((opt) => opt * width <= MAX_CELLS);
...
export function getColumnOrder({qColumnOrder, qDimensionInfo, qMeasureInfo}) {
  ...
  return qColumnOrder?.length === columnsLength
    ? qColumnOrder
...
export function getTotalInfo(isDim, layout, colIndex, numDims, columnOrder) {
  if (!isDim) {
    return layout.qHyperCube.qGrandTotalRow[colIndex - numDims]?.qText;
  }
  if (colIndex === 0 && columnOrder[0] === 0) {
...
export function getColumnInfo(layout, colIndex, columnOrder) {
  ...
  const info = isDim
    ? qDimensionInfo[colIndex]
    : qMeasureInfo[colIndex - numDims];
  ...
  return (
    !isHidden && {
      ...
      id: `col-${colIndex}`,
      align:
        !info.textAlign || info.textAlign.auto
          ? autoAlign
...
export function getTotalPosition(layout) {
  ...
  if (
    hasGrandTotal &&
    ((hasDimension && hasMeasure) || (!isTotalModeAuto && hasOnlyMeasure))
  ) {
    if (
      isTotalModeAuto ||
      (!isTotalModeAuto && layout.totals.position === 'top')
    ) {
      return 'top';
    }
    if (!isTotalModeAuto && layout.totals.position === 'bottom') {
      return 'bottom';
    }
  }
  return 'noTotals';
...
const getExpressionColor = (col, row, key) => {
  if (col.stylingInfo?.length > 0 && row.qAttrExps?.qValues?.length > 0) {
    ...
    if (bgIndex !== -1 && bgIndex < row.qAttrExps.qValues.length) {
      const rgbString = resolveToARGBorRGB(
        row.qAttrExps.qValues[bgIndex].qText,
      );
      if (rgbString !== 'none') {
...
export function resolveToARGBorRGB(input) {
  // rgb
  let matches =
    /^rgb\(\s*(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d{1,3})\s*\)$/i.exec(input);
  if (matches) {
  ...
  // argb
  matches =
    /^argb\(\s*(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d{1,3})\s*\)$/i.exec(
      input,
    );
  if (matches) {
...
const getCellBackgroundColor = (col, row) => {
  const cellBackgroundColor = getExpressionColor(
    col,
    row,
    'cellBackgroundColor',
  );
  ...
  if (
    cellBackgroundColor &&
    isDarkColor(cellBackgroundColor) &&
    !cellForegroundColor
  ) {
...
const validateImageUrl = (row, col, colIdx, imageOrigins) => {
  if (col?.representation?.imageUrl) {
    const {imageUrl} = col.representation;
    const allowed = imageOrigins.some((s) => imageUrl.includes(s));
    if (!allowed) {
      col.representation.imageUrl = null;
    }
  }
  if (col?.stylingInfo) {
...
export default async function manageData(
  ...
  if (height * totalColumnCount > MAX_CELLS && pageInfo) {
    setPageInfo({
      ...pageInfo,
      rowsPerPage: getHighestPossibleRpp(totalColumnCount, rowsPerPageOptions),
      page: 0,
    });
    return null;
  }
  ...
  const rows = dataPages[0]?.qMatrix.map((r, rowIdx) => {
    ...
    columns.forEach((c, colIdx) => {
      ...
      row[c.id] = {
        ...
        rawColIdx: colIdx,
        prevQElemNumber:
          dataPages[0].qMatrix[rowIdx - 1]?.[colIdx]?.qElemNumber,
        nextQElemNumber:
          dataPages[0].qMatrix[rowIdx + 1]?.[colIdx]?.qElemNumber,
