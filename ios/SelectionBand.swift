...
class SelectionBand: UIView {
  ...
  override init(frame: CGRect) {
    ...
    selectionBandResizer.layer.borderWidth = 2.0
  ...
  fileprivate func updateDragBox(selectionBandResizer: UIView) {
    ...
    let bottomRightPoint = CGPoint(x: selectionBandResizer.frame.origin.x ,
                                   y: selectionBandResizer.frame.origin.y + selectionBandResizer.frame.height - dragBoxSize.height / 2 )
  ...
  @objc func handleGesture(_ sender: UIPanGestureRecognizer) {
    switch sender.state {
    case .began:
      touchDownPoint = sender.location(in: self)
  ...
  fileprivate func handleDrag() {
    // get height
    guard let selectionBandResizer = self.selectionBandResizer else { return }
    let height = (currentPoint.y - selectionBandResizer.frame.origin.y)
    let newFrame = CGRect(origin: selectionBandResizer.frame.origin, size: CGSize(width: selectionBandResizer.frame.width, height: height))
